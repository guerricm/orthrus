...
public class ScanCommand implements Callable<Integer> {
	...
	@Option(names = { "-f", "--format" }, description = "Report format (json, sarif, html, pdf, console)",
	...
	@Option(names = { "--auth-bearer-secondary" },
			description = "Secondary Bearer token for Cross-User BOLA testing (User B)")
	...
	@Option(names = { "-c", "--concurrency" }, description = "Number of concurrent threads for scanning (default: 10)",
			defaultValue = "10")
	int concurrency;
	...
	@Option(names = { "--app-url" },
			description = "Public Application URL for Gateway Discovery (e.g. http://myapp.com)")
	String appUrl;
