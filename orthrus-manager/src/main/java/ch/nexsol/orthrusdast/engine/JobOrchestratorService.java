...
public class JobOrchestratorService {
	...
	public JobOrchestratorService(ScanJobRepository scanJobRepository, ScanTaskRepository scanTaskRepository, ObjectMapper objectMapper, ch.nexsol.orthrusdast.engine.ScanResultService scanResultService, ch.nexsol.orthrusdast.sse.JobEventPublisher jobEventPublisher) {
	...
	public Mono<Void> processPendingJobs() {
		return scanJobRepository.findByStatus(JobStatus.PENDING)
			.flatMap(job -> {
	...
	private Mono<Void> createFamilyTasks(ScanJobEntity job) {
		return Flux.fromArray(ScannerFamily.values())
	...
	public Mono<Void> onScanTaskComplete(Long taskId, int testsCount, int vulnsCount) {
		return scanTaskRepository.findById(taskId).flatMap(task -> {
			log.info("Scan task {} (Family: {}) completed. {} tests executed, {} vulnerabilities found.", taskId, task.getPhase(), testsCount, vulnsCount);
	...
	private Mono<Void> failJob(Long jobId, String reason) {
		return scanJobRepository.findById(jobId).flatMap(job -> {
			...
			return scanJobRepository.save(job).doOnSuccess(j -> {
				jobEventPublisher.emit(jobId, ch.nexsol.orthrusdast.sse.JobEvent.failed(jobId, job.getTarget(), reason));
		...
		return scanTaskRepository.countActiveTasksForJob(jobId).flatMap(activeCount -> {
			if (activeCount == 0) {
				...
				return scanJobRepository.findById(jobId).flatMap(job -> {
					...
					return scanResultService.finalizeJobResult(job.getResultId(), job.getTarget(), job.getStartedAt(), job.getCompletedAt(), testsCount)
						.flatMap(result -> scanJobRepository.save(job).doOnSuccess(j -> {
							...
							jobEventPublisher.emit(jobId,
									ch.nexsol.orthrusdast.sse.JobEvent.completed(jobId, job.getTarget(), result.id(), grade, result.vulnerabilities().size(),
