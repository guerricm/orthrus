...
public class SqlInjectionScanner implements SecurityScanner {
    ...
    public Flux<Vulnerability> scan(Operation operation) {
        ...
        return oastService.createSession().flatMapMany(oastSession -> {
            ...
            if (operation.queryParams() != null && !operation.queryParams().isEmpty()) {
                scanVulns = Flux.fromIterable(operation.queryParams().keySet())
                        .concatMap(paramName -> 
                            payloadLoader.getPayloads("sqli")
                                    .concatMap(payload -> testParam(operation, paramName, payload, oastSession))
                        );
            }
    ...
    private Flux<Vulnerability> testParam(Operation operation, String paramName, String rawPayload, ch.hug.orthrusdast.scanner.oast.OastService.OastSession oastSession) {
                ...
                .flatMapMany(response -> {
                    ...
                    if (response.responseTimeMs() > 4000) {
                         Vulnerability vuln = createVulnerabilityWithTrace(
                            ...
                            "The endpoint took " + response.responseTimeMs() + "ms to respond, indicating a potential Time-Based Blind SQL Injection in parameter '" + paramName + "'.",
                            RiskLevel.CRITICAL,
                            Vulnerability.Confidence.HIGH,
                            operation,
                            CWEReference.CWE_89,
                    ...
                    if (hasSqlError) {
                         Vulnerability vuln = createVulnerabilityWithTrace(
                            ...
                            "The endpoint might be vulnerable to Error-Based SQL Injection in parameter '" + paramName + "'.",
                            RiskLevel.HIGH,
