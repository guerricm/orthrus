        ...
        return httpClient.send(testOp).flatMapMany(response -> {
            ...
            if (response.isSuccessful()) {
                Vulnerability vuln = createVulnerabilityWithTrace(
                        ...
                        "Implement strict file type validation (using magic bytes, not just extensions). Use an antivirus scanner on all uploaded files. Store files outside the web root.", testOp, response,
