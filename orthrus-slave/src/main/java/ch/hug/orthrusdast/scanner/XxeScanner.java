        ...
        return httpClient.send(testOp).flatMapMany(response -> {
            ...
            if (response.bodyContains("root:x:0:0:") || response.bodyContains("daemon:x:1:1:")) {
                Vulnerability vuln = createVulnerabilityWithTrace(
                        ...
                        "Disable external entity parsing in your XML parser configuration. For Java, configure DocumentBuilderFactory to disallow DOCTYPE declarations.", testOp, response,
