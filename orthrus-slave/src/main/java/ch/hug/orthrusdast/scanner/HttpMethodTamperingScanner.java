                ...
                .flatMapMany(response -> {
                    ...
                    if ("TRACE".equals(method) && response.isSuccessful() && response.bodyContainsExact("TRACE /")) {
                        Vulnerability vuln = createVulnerabilityWithTrace(
                                ...
                                "Disable the HTTP TRACE method on the web server.", testOp, response,
                        ...
                        return Flux.just(vuln);
                         ...
                         Vulnerability vuln = createVulnerabilityWithTrace(
                                ...
                                "Restrict accepted HTTP methods to only those strictly necessary (e.g., GET, POST, PUT, DELETE).", testOp, response,
                                    "API Endpoint (Network)",
                                    "Unauthorized Access / Data Exposure");
                        return Flux.just(vuln);
                    }
