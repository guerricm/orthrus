...
class WellKnownDiscovererTest {
	...
	@Test
	void testGetId() {
		WellKnownDiscoverer discoverer = new WellKnownDiscoverer(
				org.mockito.Mockito.mock(ch.nexsol.orthrusdast.http.ScanHttpClient.class));
		assertEquals("well-known", discoverer.getId());
	}

	@Test
	void testDiscoverInvalidUrl() {
		ch.nexsol.orthrusdast.http.ScanHttpClient mockClient = org.mockito.Mockito
		...
		org.mockito.Mockito.when(mockClient.send(org.mockito.ArgumentMatchers.any()))
			.thenReturn(reactor.core.publisher.Mono
				.just(new ch.nexsol.orthrusdast.http.ScanHttpResponse(org.springframework.http.HttpStatus.NOT_FOUND,
						new org.springframework.http.HttpHeaders(), "", 0L)));
