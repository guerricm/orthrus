...
public class OpenRedirectScanner implements SecurityScanner {
	...
	private Flux<Vulnerability> testParam(Operation operation, String paramName) {
		Mono<String> targetHostMono = Mono.fromCallable(() -> {
			URI uri = new URI(operation.url());
			String host = uri.getHost();
			return (host != null) ? host : "target.com";
		}).onErrorReturn("target.com");

		return targetHostMono.flatMapMany((targetHost) -> {
			...
			return Flux.fromIterable(payloads).concatMap((payload) -> {
				...
				Operation testOp = new Operation(operation.url(), operation.method(), operation.headers(),
						modifiedParams, operation.body(), operation.securityRequirements(),
						operation.expectedContentTypes(), operation.authScheme());

				return httpClient.send(testOp).flatMapMany((response) -> {
					// Check if the response is a 3xx redirect AND the Location header
					// matches
					// the malicious URL
