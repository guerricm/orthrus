		...
		return Flux.defer(() -> {
			...
			return oastService.createSession().flatMapMany((oastSession) -> {
				...
				Operation testOpPasswd = new Operation(operation.url(), operation.method(), newHeaders,
						operation.queryParams(), XXE_PAYLOAD, operation.securityRequirements(),
						operation.expectedContentTypes(), operation.authScheme());

				Operation testOpOast = new Operation(operation.url(), operation.method(), newHeaders,
						operation.queryParams(), oastXxePayload, operation.securityRequirements(),
						operation.expectedContentTypes(), operation.authScheme());
				...
				Flux<Vulnerability> scanVulns = httpClient.send(testOpPasswd).flatMapMany((response) -> {
					if (response.bodyContains("root:x:0:0:") || response.bodyContains("daemon:x:1:1:")) {
						Vulnerability vuln = createVulnerabilityWithTrace("XML External Entity (XXE) Injection",
								...
								List.of("CAPEC-228"), 9.8, "The response contained the contents of /etc/passwd.",
								...
								testOpPasswd, response, "API Endpoint (Network)",
