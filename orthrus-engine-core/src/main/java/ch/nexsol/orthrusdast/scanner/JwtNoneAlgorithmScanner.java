...
public class JwtNoneAlgorithmScanner implements SecurityScanner {
    ...
    public Flux<Vulnerability> scan(Operation operation) {
        ...
        return Flux.just("none", "None", "NONE")
                .concatMap(noneAlg -> {
                            ...
                            .flatMapMany(response -> {
                                if (response.isSuccessful() && !response.bodyContains("Unauthorized") && !response.bodyContains("Unauthenticated") && !response.bodyContains("invalid token")) {
                                    Vulnerability vuln = createVulnerabilityWithTrace(
                                            ...
                                            "The endpoint accepts JWTs signed with the 'none' algorithm (using variation '" + noneAlg + "'), allowing authentication bypass.",
                                            RiskLevel.CRITICAL,
                                            ...
                                            List.of("CAPEC-115"),
                                            9.8,
                                            "Endpoint returned " + response.statusCode() + " OK when a JWT with 'alg: " + noneAlg + "' was provided.",
                                            "Configure your JWT library to explicitly reject the 'none' algorithm and enforce expected algorithms.", testOp, response,
