...
public class SsrfScanner implements SecurityScanner {
    ...
    public Flux<Vulnerability> scan(Operation operation) {
        return oastService.createSession().flatMapMany(session -> {
            ...
            Flux<Vulnerability> errorBasedVulns = Flux.fromIterable(payloads).concatMap(payload -> {
                return InjectionHelper.generateInjectedOperations(operation, payload).concatMap(test -> 
                    httpClient.send(test.mutatedOperation()).flatMapMany(response -> {
                        if (response.bodyContains("ami-id") || response.bodyContains("instance-id") || response.bodyContains("local-hostname") || response.bodyContains("root:x:0:0")) {
            ...
            Flux<Vulnerability> blindVulns = oastService.pollInteractions(session).map(interaction -> 
                createVulnerabilityWithTrace(
                    ...
                    List.of("CAPEC-664"),
                    8.6,
                    "Received an interaction from " + interaction.remoteAddress() + " via " + interaction.protocol() + " to the injected OAST domain.",
