...
public class HttpMethodTamperingScanner implements SecurityScanner {
    ...
    public Flux<Vulnerability> scan(Operation operation) {
        ...
        if ("POST".equals(originalMethod) || "PUT".equals(originalMethod) || "DELETE".equals(originalMethod) || "PATCH".equals(originalMethod)) {
            // Send as GET but with X-HTTP-Method-Override: POST
            Map<String, String> overrideHeaders = new HashMap<>(operation.headers() != null ? operation.headers() : new HashMap<>());
            ...
            Operation overrideOp = new Operation(
                    operation.url(), "GET", overrideHeaders, operation.queryParams(),
            ...
            overrideVulns = httpClient.send(overrideOp).flatMapMany(response -> {
                if (response.isSuccessful()) {
                    Vulnerability vuln = createVulnerabilityWithTrace(
                            ...
                            "The endpoint accepted a GET request but processed it as a " + originalMethod + " because of the X-HTTP-Method-Override headers. This can bypass WAF rules or naive authorization checks that only inspect the request line method.",
                            RiskLevel.MEDIUM, Vulnerability.Confidence.MEDIUM, operation, CWEReference.CWE_650, List.of("CAPEC-274"), 5.3,
                            "Server responded with " + response.statusCode() + " when sending a GET request with X-HTTP-Method-Override: " + originalMethod + ".",
                            "Disable support for HTTP method override headers unless strictly necessary for legacy clients.", overrideOp, response, "API Endpoint (Network)", "Unauthorized Access / Data Exposure");
                ...
                .flatMapMany(response -> {
                    ...
                    if ("TRACE".equals(method) && response.isSuccessful() && response.bodyContains("TRACE /")) {
                        Vulnerability vuln = createVulnerabilityWithTrace(
                                ...
                                RiskLevel.MEDIUM, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_650, List.of("CAPEC-274"), 5.3,
                                "Server responded with 200 OK and reflected the request when using the TRACE method.",
                                "Disable the HTTP TRACE method on the web server.", testOp, response, "API Endpoint (Network)", "Unauthorized Access / Data Exposure");
                        return Flux.just(vuln);
                         ...
                         Vulnerability vuln = createVulnerabilityWithTrace(
                                ...
                                RiskLevel.LOW, Vulnerability.Confidence.MEDIUM, operation, CWEReference.CWE_650, List.of("CAPEC-274"), 5.3,
                                "Server responded with " + response.statusCode() + " when using the " + method + " method.",
                                "Restrict accepted HTTP methods to only those strictly necessary (e.g., GET, POST, PUT, DELETE).", testOp, response, "API Endpoint (Network)", "Unauthorized Access / Data Exposure");
                        return Flux.just(vuln);
                    }
                    return Flux.empty();
                });
