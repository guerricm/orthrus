...
public class SslCertificateScanner implements SecurityScanner {
        ...
        try {
            ...
            try (SSLSocket socket = (SSLSocket) factory.createSocket(hostname, port)) {
                ...
                if ("TLSv1".equals(protocol) || "TLSv1.1".equals(protocol) || "SSLv3".equals(protocol) || "SSLv2".equals(protocol)) {
                    vulnerabilities.add(createVulnerabilityWithTrace(
                            ...
                            "Disable outdated protocols like TLS 1.0 and TLS 1.1. Enforce TLS 1.2 or TLS 1.3.", operation, null,
                ...
                if (peerCerts.length > 0 && peerCerts[0] instanceof X509Certificate cert) {
                    ...
                    try {
                        ...
                        vulnerabilities.add(createVulnerabilityWithTrace(
                                ...
                                "Renew and deploy a valid SSL certificate.", operation, null,
                                    "API Endpoint (Network)",
                                    "Unauthorized Access / Data Exposure"));
                    ...
                    if (cert.getIssuerX500Principal().equals(cert.getSubjectX500Principal())) {
                        vulnerabilities.add(createVulnerabilityWithTrace(
                                ...
                                "Use a certificate signed by a trusted Certificate Authority (CA) for production environments.", operation, null,
                                    "API Endpoint (Network)",
                                    "Unauthorized Access / Data Exposure"));
                    }
                    ...
                    if (sigAlg.contains("MD5") || sigAlg.contains("SHA1")) {
                        vulnerabilities.add(createVulnerabilityWithTrace(
                                ...
                                RiskLevel.MEDIUM, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_327, List.of(), 5.9,
                                "The certificate was signed using " + sigAlg + ".",
                                "Re-issue the certificate using a strong hashing algorithm like SHA-256.", operation, null, "API Endpoint (Network)", "Unauthorized Access / Data Exposure"));
                    }
                    ...
                    if (pubKey instanceof RSAPublicKey rsaPubKey) {
                        ...
                        if (keySize < 2048) {
                            vulnerabilities.add(createVulnerabilityWithTrace(
                                    "Weak RSA Key Length",
                                    "The SSL certificate uses an RSA key size of " + keySize + " bits, which is considered cryptographically weak.",
                                    RiskLevel.MEDIUM, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_200, List.of(), 5.9,
                                    "The RSA public key size is " + keySize + " bits.",
                                    "Use an RSA key size of at least 2048 bits, or switch to modern elliptic curve cryptography (ECC).", operation, null, "API Endpoint (Network)", "Unauthorized Access / Data Exposure"));
                        }
                    }
                    ...
                    if (!hostnameMatches(hostname, cert)) {
                        vulnerabilities.add(createVulnerabilityWithTrace(
                                ...
                                "The SSL certificate presented by the server is not valid for the requested hostname (" + hostname + ").",
                                RiskLevel.HIGH, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_200, List.of("CAPEC-475"), 6.5,
                                "The requested hostname '" + hostname + "' does not match the CN or SANs in the certificate.",
                                "Ensure the certificate is issued for the correct hostname, including any necessary Subject Alternative Names (SANs).", operation, null, "API Endpoint (Network)", "Unauthorized Access / Data Exposure"));
                    }
                }
            }
        } catch (Exception e) {
    ...
    private boolean hostnameMatches(String hostname, X509Certificate cert) {
        ...
        String cn = null;
        try {
            javax.naming.ldap.LdapName ldapDN = new javax.naming.ldap.LdapName(cert.getSubjectX500Principal().getName());
