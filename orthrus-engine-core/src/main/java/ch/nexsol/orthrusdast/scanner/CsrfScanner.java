...
public class CsrfScanner implements SecurityScanner {
    ...
    public Flux<Vulnerability> scan(Operation operation) {
        ...
        java.util.Map<String, String> noTokenHeaders = new java.util.HashMap<>(operation.headers() != null ? operation.headers() : new java.util.HashMap<>());
        noTokenHeaders.keySet().removeIf(k -> k.toLowerCase().contains("csrf") || k.toLowerCase().contains("xsrf") || k.equalsIgnoreCase("X-Requested-With"));
        noTokenHeaders.put("Origin", "https://malicious-website.com");

        Operation testOpNoToken = new Operation(
                operation.url(), operation.method(), noTokenHeaders, operation.queryParams(),
                operation.body(), operation.securityRequirements(), operation.expectedContentTypes(), operation.authScheme()
        );

        // Strategy 2: Content-Type bypass. Send text/plain instead of application/json to bypass preflight CORS checks.
        java.util.Map<String, String> plainHeaders = new java.util.HashMap<>(noTokenHeaders);
        plainHeaders.put("Content-Type", "text/plain");

        Operation testOpPlain = new Operation(
                operation.url(), operation.method(), plainHeaders, operation.queryParams(),
                operation.body(), operation.securityRequirements(), operation.expectedContentTypes(), operation.authScheme()
        );

        return Flux.concat(
            httpClient.send(testOpNoToken).flatMapMany(response -> {
                if (response.isSuccessful()) {
                    Vulnerability vuln = createVulnerabilityWithTrace(
                            ...
                            RiskLevel.MEDIUM, Vulnerability.Confidence.LOW, operation, CWEReference.CWE_352, List.of("CAPEC-62"), 6.5,
                            "Server accepted the request with Origin: https://malicious-website.com and no CSRF tokens.",
                            "Implement Anti-CSRF tokens (Synchronizer Token Pattern) for all state-changing endpoints if using Cookie authentication. Ensure cookies have the SameSite=Lax or Strict attribute.", testOpNoToken, response,
            ...
            httpClient.send(testOpPlain).flatMapMany(response -> {
                if (response.isSuccessful()) {
                    Vulnerability vuln = createVulnerabilityWithTrace(
                            ...
                            RiskLevel.HIGH, Vulnerability.Confidence.MEDIUM, operation, CWEReference.CWE_352, List.of("CAPEC-62"), 7.5,
                            ...
                            "Do not rely solely on Content-Type checks or CORS preflights for CSRF protection. Implement explicit CSRF tokens or rely on SameSite cookie attributes.", testOpPlain, response,
                            "API Endpoint (Network)", "Unauthorized Access / Data Exposure");
                    return Flux.just(vuln);
                }
                return Flux.empty();
