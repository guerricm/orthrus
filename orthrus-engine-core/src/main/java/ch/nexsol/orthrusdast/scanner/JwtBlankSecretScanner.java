...
public class JwtBlankSecretScanner implements SecurityScanner {
	...
	public Flux<Vulnerability> scan(Operation operation) {
		return Flux.defer(() -> {
			...
			return Flux.fromIterable(WEAK_SECRETS).concatMap((secret) -> {
				try {
					...
					return httpClient.send(testOp).flatMapMany((response) -> {
						if (response.isSuccessful() && !response.bodyContains("Unauthorized")
								...
								&& !response.bodyContains("invalid token")) {
							...
							Vulnerability vuln = createVulnerabilityWithTrace(title, description, RiskLevel.CRITICAL,
									Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_287,
									List.of("CAPEC-115"), 9.8,
									"Endpoint returned " + response.statusCode()
											+ " OK when a forged JWT was provided.",
