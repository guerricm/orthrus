    ...
    public Flux<Vulnerability> scan(Operation operation) {
        ...
        return oastService.createSession().flatMapMany(oastSession -> {
            try {
                ...
                Flux<String> allPayloads = Flux.concat(
                        payloadLoader.getPayloads("sqli"),
                        payloadLoader.getPayloads("cmd-injection"),
                        payloadLoader.getPayloads("nosql"),
                        payloadLoader.getPayloads("ssti")
                );

                Flux<Vulnerability> scanVulns = Flux.fromIterable(variables.keySet())
                        .flatMap(varName -> allPayloads
                                .concatMap(rawPayload -> {
                                    String oastPayload = rawPayload.replace("{{OAST_HOST}}", oastSession.domain());
                                    String payload = payloadMutator.mutate(oastPayload, ch.nexsol.orthrusdast.scanner.payload.PayloadMutator.Context.JSON_BODY);
                ...
                return scanVulns.concatWith(oastService.pollInteractions(oastSession).map(interaction -> 
                    createVulnerabilityWithTrace(
                            "Out-Of-Band (Blind) Injection in GraphQL",
                            ...
                            operation,
                            CWEReference.CWE_89, // Defaulting to 89, though could be 78 or SSRF
                            List.of("CAPEC-66"),
                            9.8,
                    ...
                    .flatMapMany(response -> {
                        if (response.responseTimeMs() > 4000 && payload.contains("sleep")) {
                            Vulnerability vuln = createVulnerabilityWithTrace(
                                    ...
                                    List.of("CAPEC-66"),
                                    9.8,
                                    "Response was delayed by " + response.responseTimeMs() + "ms when payload '" + payload + "' was supplied.",
                        ...
                        boolean hasError = body.contains("syntax error") || body.contains("mysql_fetch") || body.contains("ora-") || body.contains("command not found");
                        
                        if (hasError) {
                            Vulnerability vuln = createVulnerabilityWithTrace(
                                    "Potential Injection in GraphQL Variable",
                                    ...
                                    List.of("CAPEC-66"),
                                    9.8,
                                    "Response indicates a potential injection error when payload '" + payload + "' was supplied.",
                                    "Validate and sanitize all GraphQL variable inputs. Use parameterized queries for databases.", testOp, response,
                                    "API Endpoint (Network)",
                                    "Unauthorized Access / Data Exposure");
                            return Flux.just(vuln);
