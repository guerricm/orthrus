...
public class ContentTypeSpoofingScanner implements SecurityScanner {
	...
	public Flux<Vulnerability> scan(Operation operation) {
		return Flux.defer(() -> {
			...
			Operation testOpXml = new Operation(operation.url(), operation.method(), xmlHeaders,
					operation.queryParams(), XML_PAYLOAD, operation.securityRequirements(),
			...
			xxeVulns = httpClient.send(testOpXml).flatMapMany((response) -> {
				if (response.bodyContainsExact("root:x:0:0")) {
					Vulnerability vuln = createVulnerabilityWithTrace(
							...
							RiskLevel.CRITICAL, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_611,
							List.of("CAPEC-228"), 7.5,
							...
							testOpXml, response, "API Endpoint (Network)", "Unauthorized Access / Data Exposure");
					return Flux.just(vuln);
				}
				if (response.statusCode().is5xxServerError()) {
					Vulnerability vuln = createVulnerabilityWithTrace("Unhandled Exception on Unexpected Content-Type",
							"The endpoint throws a 500 Server Error when provided with an unexpected Content-Type.",
							RiskLevel.LOW, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_209,
							List.of("CAPEC-228"), 3.5,
			...
			if (operation.body() != null && !operation.body().isEmpty()) {
				...
				Operation testOpText = new Operation(operation.url(), operation.method(), textHeaders,
						operation.queryParams(), operation.body(), operation.securityRequirements(),
						operation.expectedContentTypes(), operation.authScheme());

				bypassVulns = httpClient.send(operation)
					.flatMapMany((originalResponse) -> httpClient.send(testOpText).flatMapMany((testResponse) -> {
						if (originalResponse.isSuccessful() && testResponse.isSuccessful()
								&& originalResponse.statusCode().equals(testResponse.statusCode())) {
							// If sending text/plain succeeds just like application/json,
							// it might bypass WAFs looking for JSON
							Vulnerability vuln = createVulnerabilityWithTrace(
									...
									RiskLevel.LOW, Vulnerability.Confidence.MEDIUM, operation, CWEReference.CWE_434,
									List.of("CAPEC-228"), 4.3,
									"Server responded with " + testResponse.statusCode()
											+ " for both application/json and text/plain.",
									"Strictly enforce 'Content-Type: application/json' and reject (415 Unsupported Media Type) any other content types.",
									testOpText, testResponse, "API Endpoint (Network)",
