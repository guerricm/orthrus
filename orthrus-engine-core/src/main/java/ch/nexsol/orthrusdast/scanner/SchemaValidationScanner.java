 ...
 * It verifies if the API actually enforces required fields, max lengths, bounds, enums,
...
public class SchemaValidationScanner implements SecurityScanner {
    ...
    private List<Flux<Vulnerability>> generateRequestBodyTests(Operation operation, Schema schema) {
        ...
        if (schema.getProperties() == null) return scans;
        ...
        for (Map.Entry<String, Schema> entry : properties.entrySet()) {
            ...
            if (schema.getRequired() != null && schema.getRequired().contains(propName)) {
                ...
                scans.add(testBodyPayload(operation, mutated, "Missing Required Property", 
                    "The property '" + propName + "' is required by the OpenAPI schema, but omitting it did not result in a 400 Bad Request error."));
            }
            ...
            if ("string".equals(propSchema.getType())) {
                ...
                if (propSchema.getMaxLength() != null) {
                    ...
                    scans.add(testBodyPayload(operation, mutated, "Max Length Violation", 
                        "The property '" + propName + "' has a maxLength of " + propSchema.getMaxLength() + ", but providing a longer string was accepted."));
                } else {
                    ...
                    scans.add(testBodyPayload(operation, mutated, "Missing Max Length (Unbounded String DoS)", 
                        "The property '" + propName + "' lacks a maxLength constraint. An abnormally large string (50 MB) was sent and caused a 2xx or 5xx response, indicating missing bounds checking and potential DoS vulnerability."));
                }
                // Min Length
                if (propSchema.getMinLength() != null && propSchema.getMinLength() > 0) {
                    ...
                    scans.add(testBodyPayload(operation, mutated, "Min Length Violation", 
                        "The property '" + propName + "' has a minLength of " + propSchema.getMinLength() + ", but providing a shorter string was accepted."));
                }
                // Pattern
                if (propSchema.getPattern() != null) {
                    Map<String, Object> mutated = new HashMap<>(baseline);
                    mutated.put(propName, "invalid_pattern_string_!@#$%^&*()");
                    scans.add(testBodyPayload(operation, mutated, "Pattern Constraint Violation", 
                        "The property '" + propName + "' expects a specific regex pattern, but an invalid string was accepted."));
                }
                // Enum
                if (propSchema.getEnum() != null && !propSchema.getEnum().isEmpty()) {
                    Map<String, Object> mutated = new HashMap<>(baseline);
                    mutated.put(propName, "ORTHRUS_INVALID_ENUM_VALUE");
                    scans.add(testBodyPayload(operation, mutated, "Enum Constraint Violation", 
                        "The property '" + propName + "' defines an enum, but a value outside the allowed list was accepted."));
                }
                // Format
                if (propSchema.getFormat() != null) {
                    Map<String, Object> mutated = new HashMap<>(baseline);
                    if ("email".equals(propSchema.getFormat())) mutated.put(propName, "not-an-email-address");
                    else if ("uuid".equals(propSchema.getFormat())) mutated.put(propName, "not-a-uuid-1234");
                    else if ("date".equals(propSchema.getFormat())) mutated.put(propName, "2024-13-45");
                    scans.add(testBodyPayload(operation, mutated, "Format Constraint Violation", 
                        "The property '" + propName + "' expects format '" + propSchema.getFormat() + "', but an invalid format was accepted."));
                }
            }
            ...
            if ("integer".equals(propSchema.getType()) || "number".equals(propSchema.getType())) {
                ...
                if (propSchema.getMaximum() != null) {
                    ...
                    scans.add(testBodyPayload(operation, mutated, "Maximum Constraint Violation", 
                        "The property '" + propName + "' has a maximum of " + propSchema.getMaximum() + ", but a larger value was accepted."));
                }
                // Minimum
                if (propSchema.getMinimum() != null) {
                    ...
                    scans.add(testBodyPayload(operation, mutated, "Minimum Constraint Violation", 
                        "The property '" + propName + "' has a minimum of " + propSchema.getMinimum() + ", but a smaller value was accepted."));
                }
            }
            ...
            if ("array".equals(propSchema.getType())) {
                if (propSchema.getMaxItems() != null) {
                    ...
                    for (int i = 0; i <= propSchema.getMaxItems(); i++) list.add("item");
                    mutated.put(propName, list);
                    scans.add(testBodyPayload(operation, mutated, "Max Items Constraint Violation", 
                        "The array property '" + propName + "' has maxItems " + propSchema.getMaxItems() + ", but a larger array was accepted."));
                }
                if (propSchema.getMinItems() != null && propSchema.getMinItems() > 0) {
                    Map<String, Object> mutated = new HashMap<>(baseline);
                    List<String> list = new ArrayList<>();
                    for (int i = 0; i < propSchema.getMinItems() - 1; i++) list.add("item");
                    mutated.put(propName, list);
                    scans.add(testBodyPayload(operation, mutated, "Min Items Constraint Violation", 
                        "The array property '" + propName + "' has minItems " + propSchema.getMinItems() + ", but a smaller array was accepted."));
                }
            }
        }
        return scans;
    ...
    private List<Flux<Vulnerability>> generateParameterTests(Operation baseOp, List<Parameter> parameters) {
        ...
        for (Parameter param : parameters) {
            if (param.getSchema() == null) continue;
            String pName = param.getName();
            Schema pSchema = param.getSchema();
            
            // Missing Required Parameter (Only for Query and Header, Path is implicitly required by URL)
            if (Boolean.TRUE.equals(param.getRequired()) && ("query".equals(param.getIn()) || "header".equals(param.getIn()))) {
                Operation mutatedOp = removeParameter(baseOp, param.getIn(), pName);
                scans.add(testOperation(mutatedOp, "Missing Required Parameter", 
                    "The parameter '" + pName + "' is required in " + param.getIn() + " but was omitted, and the server did not return 400."));
            }

            // Constraints for String Parameters
            if ("string".equals(pSchema.getType())) {
                if (pSchema.getMaxLength() != null) {
                    scans.add(testOperation(mutateParameter(baseOp, param.getIn(), pName, "A".repeat(pSchema.getMaxLength() + 10)), 
                        "Parameter Max Length Violation", "Parameter '" + pName + "' exceeded maxLength but was accepted."));
                } else {
                    scans.add(testOperation(mutateParameter(baseOp, param.getIn(), pName, "A".repeat(50_000_000)), 
                        "Missing Parameter Max Length (Unbounded)", "Parameter '" + pName + "' lacks a maxLength constraint. An abnormally large string (50 MB) was accepted, potential DoS."));
                }
                if (pSchema.getEnum() != null && !pSchema.getEnum().isEmpty()) {
                    scans.add(testOperation(mutateParameter(baseOp, param.getIn(), pName, "ORTHRUS_INVALID_ENUM"), 
                        "Parameter Enum Violation", "Parameter '" + pName + "' received an invalid enum value but was accepted."));
                }
            }
            ...
            if ("integer".equals(pSchema.getType()) || "number".equals(pSchema.getType())) {
                scans.add(testOperation(mutateParameter(baseOp, param.getIn(), pName, "invalid_string_not_number"), 
                        "Parameter Type Violation", "Parameter '" + pName + "' expects a number but accepted a string."));
                        
                if (pSchema.getMaximum() != null) {
                    scans.add(testOperation(mutateParameter(baseOp, param.getIn(), pName, String.valueOf(pSchema.getMaximum().longValue() + 10)), 
                        "Parameter Maximum Violation", "Parameter '" + pName + "' exceeded maximum but was accepted."));
                }
                if (pSchema.getMinimum() != null) {
                    scans.add(testOperation(mutateParameter(baseOp, param.getIn(), pName, String.valueOf(pSchema.getMinimum().longValue() - 10)), 
                        "Parameter Minimum Violation", "Parameter '" + pName + "' was below minimum but was accepted."));
                }
            }
        }
    ...
    private Operation removeParameter(Operation op, String in, String name) {
        ...
        if ("query".equals(in)) newQuery.remove(name);
        if ("header".equals(in)) newHeaders.remove(name);
        
        return new Operation(op.url(), op.method(), newHeaders, newQuery, op.body(), op.securityRequirements(), op.expectedContentTypes(), op.authScheme(), op.templateUrl(), op.sourceNode());
    }
    ...
    private Operation mutateParameter(Operation op, String in, String name, String value) {
        ...
        if ("query".equals(in)) newQuery.put(name, value);
        if ("header".equals(in)) newHeaders.put(name, value);
        // Note: Mutating path params dynamically requires rewriting the URL which is complex, so we stick to query/header for now.
    ...
    private Flux<Vulnerability> testBodyPayload(Operation operation, Map<String, Object> payloadMap, String testName, String description) {
        String payloadJson;
        try {
        ...
        return httpClient.send(testOp).flatMapMany(response -> {
            ...
            if (response.statusCode().is2xxSuccessful() || response.statusCode().is5xxServerError()) {
                Vulnerability vuln = createVulnerabilityWithTrace(
                        ...
                        "Ensure that all incoming data is strictly validated against the defined OpenAPI schema constraints (types, lengths, required fields) using a validation middleware before processing.", testOp, response,
