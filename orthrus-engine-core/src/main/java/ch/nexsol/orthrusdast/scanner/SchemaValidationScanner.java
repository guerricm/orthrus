...
/**
 * Scans for Improper Input Validation (CWE-20) by testing OpenAPI schema
 * constraints.
 * It verifies if the API actually enforces required fields, max lengths,
 * bounds, enums,
 * patterns, arrays limits, and data types across both the Request Body and
 * Parameters.
 */
@Component
        ...
        return Flux.defer(() -> {
            ...
            if (List.of("POST", "PUT", "PATCH").contains(operation.method().toUpperCase())) {
                if (openApiOperation.getRequestBody() != null
                    ...
                    io.swagger.v3.oas.models.media.MediaType mediaType = openApiOperation.getRequestBody().getContent()
            ...
            if ("string".equals(propSchema.getType())) {
                ...
                if (propSchema.getPattern() != null) {
                    ...
                    scans.add(testBodyPayload(operation, mutated, "Pattern Constraint Violation",
                            "The property '" + propName
                                    + "' expects a specific regex pattern, but an invalid string was accepted."));
                }
                // Enum
                if (propSchema.getEnum() != null && !propSchema.getEnum().isEmpty()) {
                    ...
                    scans.add(testBodyPayload(operation, mutated, "Enum Constraint Violation",
                            "The property '" + propName
                                    + "' defines an enum, but a value outside the allowed list was accepted."));
                }
                // Format
            ...
            if ("string".equals(pSchema.getType())) {
                if (pSchema.getMaxLength() != null) {
                    scans.add(testOperation(
                            mutateParameter(baseOp, param.getIn(), pName, "A".repeat(pSchema.getMaxLength() + 10)),
