...
public class XssScanner implements SecurityScanner {
			...
			return oastService.createSession().flatMapMany((oastSession) -> {
				...
				return scanVulns.concatWith(oastService.pollInteractions(oastSession)
					.map((interaction) -> createVulnerabilityWithTrace("Out-Of-Band (Blind) XSS",
							...
							RiskLevel.CRITICAL, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_79,
	...
	private Flux<Vulnerability> executeAndCheck(Operation testOp, Operation originalOp, String injectionPoint,
			String payload) {
		return httpClient.send(testOp).flatMapMany((response) -> {
			if (response.bodyContainsExact(payload)) {
				...
				// Basic context detection: if it's HTML, check if characters like '<',
				// '>', or '"' are present and unescaped
				boolean isUnescaped = payload.contains("<") && response.bodyContainsExact("<")
						|| payload.contains("\"") && response.bodyContainsExact("\"");

				if (!isUnescaped) {
					// If the payload was safely HTML-encoded (e.g. &lt;), it's not a
				...
				String severityContext = isHtml
						? "The response is served as HTML, meaning a browser will execute this script directly."
						: (isJson
								? "The response is served as JSON. Modern browsers will not execute scripts in JSON, but it may pose a risk if the frontend incorrectly parses and renders this data into the DOM."
								: "The response is not HTML, so the script won't execute directly in modern browsers, but could be dangerous if a frontend incorrectly inserts this data into the DOM.");

				Vulnerability vuln = createVulnerabilityWithTrace("Reflected Cross-Site Scripting (XSS)",
						"The endpoint reflects unencoded user input from " + injectionPoint + " into the response.",
						risk, Vulnerability.Confidence.HIGH, originalOp, CWEReference.CWE_79, List.of("CAPEC-63"), 6.1,
