...
public class AuthenticationBruteForceScanner implements SecurityScanner {
    ...
    private static final Pattern USERNAME_JSON_PATTERN = Pattern.compile("(\"(?:username|user|email|login)\"\\s*:\\s*\")[^\"]*(\")", Pattern.CASE_INSENSITIVE);
    ...
    public Flux<Vulnerability> scan(Operation operation) {
        ...
        Flux<Vulnerability> bruteForceVulns = Flux.fromIterable(weakPasswords)
                .flatMap(weakPassword -> {
                    ...
                    Operation testOp = new Operation(
                            ...
                            modifiedBody, operation.securityRequirements(), operation.expectedContentTypes(), operation.authScheme()
                    );
                            ...
                            .flatMapMany(response -> {
                                ...
                                if (response.statusCode().is2xxSuccessful()) {
                                    Vulnerability vuln = createVulnerabilityWithTrace(
                                            ...
                                            RiskLevel.CRITICAL, Vulnerability.Confidence.HIGH, operation, CWEReference.CWE_521, List.of("CAPEC-112"), 9.8,
                                            "Endpoint returned " + response.statusCode() + " OK when authenticating with the weak password '" + weakPassword + "'.",
                                            "Implement strict password complexity requirements and rate limiting or account lockout mechanisms to prevent brute forcing.", testOp, response, "API Endpoint (Network)", "Unauthorized Access / Data Exposure");
        ...
        if (userMatcher.find()) {
            ...
            String validUserBody = operation.body(); // Assuming original is valid or at least exists

            Operation invalidUserOp = new Operation(
                    operation.url(), operation.method(), operation.headers(), operation.queryParams(),
                    invalidUserBody, operation.securityRequirements(), operation.expectedContentTypes(), operation.authScheme()
            );

            Operation validUserOp = new Operation(
                    operation.url(), operation.method(), operation.headers(), operation.queryParams(),
                    validUserBody, operation.securityRequirements(), operation.expectedContentTypes(), operation.authScheme()
            );

            enumerationVulns = httpClient.send(invalidUserOp).zipWith(httpClient.send(validUserOp))
                .flatMapMany(tuple -> {
                    var invalidResp = tuple.getT1();
                    var validResp = tuple.getT2();

                    // Different status codes or completely different response body lengths/content for auth failure
                    if (!invalidResp.isSuccessful() && !validResp.isSuccessful()) {
                        ...
                        if (diffStatus || diffBody) {
                            Vulnerability vuln = createVulnerabilityWithTrace(
                                    ...
                                    RiskLevel.MEDIUM, Vulnerability.Confidence.MEDIUM, operation, CWEReference.CWE_200, List.of("CAPEC-114"), 5.3,
                                    "Response to invalid username was " + invalidResp.statusCode() + " (Body length: " + invalidResp.body().length() + "), but response to original username was " + validResp.statusCode() + " (Body length: " + validResp.body().length() + ").",
                                    "Ensure authentication endpoints return a generic error message (e.g., 'Invalid username or password') and consistent HTTP status codes regardless of whether the username exists.", validUserOp, validResp, "API Endpoint (Network)", "Unauthorized Access / Data Exposure");
                            return Flux.just(vuln);
                        }
                    }
                    return Flux.empty();
