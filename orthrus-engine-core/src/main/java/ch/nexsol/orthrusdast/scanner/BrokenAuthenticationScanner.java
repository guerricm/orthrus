...
/**
 * Scans for Broken Authentication (CWE-306, CWE-287). Checks if an endpoint that usually
 * requires auth is accessible without it or with invalid auth.
 */
@Component
public class BrokenAuthenticationScanner implements SecurityScanner {
	...
	public Flux<Vulnerability> scan(Operation operation) {
		return Flux.defer(() -> {
			...
			Operation noAuthOp = new Operation(operation.url(), operation.method(), operation.headers(),
					operation.queryParams(), operation.body(), operation.securityRequirements(),
					operation.expectedContentTypes(), null);
			...
			return Flux.concat(
					executeAuthCheck(noAuthOp, operation, "Missing Authentication",
							"Endpoint returned OK without any authentication credentials."),
					executeAuthCheck(garbageAuthOp, operation, "Invalid Authentication Accepted",
							"Endpoint returned OK despite a completely invalid/garbage token."),
					executeAuthCheck(sqliAuthOp, operation, "SQLi Bypass in Authentication",
							"Endpoint returned OK when injecting SQL into the Authorization header."));
		});
	}

	private Flux<Vulnerability> executeAuthCheck(Operation testOp, Operation originalOp, String title,
			String evidence) {
		return httpClient.send(testOp).flatMapMany((response) -> {
			if (response.isSuccessful() && !response.bodyContains("Unauthorized")
					&& !response.bodyContains("Unauthenticated")) {
				Vulnerability vuln = createVulnerabilityWithTrace(title,
						"The endpoint exposes sensitive operations without properly validating authentication.",
						RiskLevel.CRITICAL, Vulnerability.Confidence.HIGH, originalOp, CWEReference.CWE_306,
						List.of("CAPEC-115"), 9.8, evidence + " Status: " + response.statusCode(),
