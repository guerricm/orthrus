...
public class GraphqlDosScanner implements SecurityScanner {
    ...
    public Flux<Vulnerability> scan(Operation operation) {
        ...
        Operation testOpIntrospection = new Operation(
                operation.url(), operation.method(), newHeaders, operation.queryParams(),
                INTROSPECTION_DOS, operation.securityRequirements(), operation.expectedContentTypes(), operation.authScheme()
        );

        Operation testOpAlias = new Operation(
                operation.url(), operation.method(), newHeaders, operation.queryParams(),
                ALIAS_DOS, operation.securityRequirements(), operation.expectedContentTypes(), operation.authScheme()
        );
        ...
        return httpClient.send(testOp).flatMapMany(response -> {
            ...
            if (response.statusCode().is5xxServerError() || duration > 3000) {
                Vulnerability vuln = createVulnerabilityWithTrace(
                        "GraphQL Denial of Service - " + payloadType,
                        "The GraphQL endpoint allowed an expensive query (" + payloadType + "), causing high response times or a server error. This indicates a lack of query cost analysis or depth limiting.",
                        RiskLevel.HIGH,
                        ...
                        List.of("CAPEC-130"),
                        7.5,
                        "Server took " + duration + "ms or returned " + response.statusCode() + " when processing the query.",
                        "Implement GraphQL query depth limiting, alias limits, and cost analysis to prevent expensive queries from consuming server resources. Disable Introspection in production.", testOp, response,
