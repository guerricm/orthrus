...
import ch.hug.orthrusdast.repository.VulnerabilityRepository;
                ...
                .flatMap(scans -> {
                    ...
                    return Flux.fromIterable(scans)
                            ...
                            .map(scanEntries -> {
                                ...
                                for (Map.Entry<ch.hug.orthrusdast.entity.ScanResultEntity, Map<String, Long>> scanEntry : scanEntries) {
                                    ch.hug.orthrusdast.entity.ScanResultEntity scan = scanEntry.getKey();
                                ...
                                vulnerabilityRepository.findByScanResultId(scan.id())
                                        ...
                                        .map(vulns -> {
                                            Map<String, Long> byRisk = vulns.stream()
                                                    ...
                                                    .collect(Collectors.groupingBy(ch.hug.orthrusdast.entity.VulnerabilityEntity::riskLevel, Collectors.counting()));
                                            
                                            Map<String, Long> byCwe = vulns.stream()
                                                    .filter(v -> v.cweId() != null)
                                                    .collect(Collectors.groupingBy(ch.hug.orthrusdast.entity.VulnerabilityEntity::cweId, Collectors.counting()));
