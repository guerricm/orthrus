...
// ch.hug.orthrusdast.engine.ScanService removed
...
import ch.hug.orthrusdast.sse.JobEvent;
import ch.hug.orthrusdast.sse.JobEventPublisher;
...
public class FrontendController {
    ...
    private final ch.hug.orthrusdast.repository.ScanJobRepository scanJobRepository;
    private final ch.hug.orthrusdast.repository.SlaveNodeRepository slaveNodeRepository;
    ...
    public FrontendController(ScanResultService scanResultService, PdfReportGenerator pdfReportGenerator,
            ...
            ch.hug.orthrusdast.repository.ScanJobRepository scanJobRepository,
            ch.hug.orthrusdast.repository.SlaveNodeRepository slaveNodeRepository,
    ...
    public Mono<String> systemStatus(Model model) {
        return Mono.zip(
                ...
                slaveNodeRepository.findAll().collectList()).map(tuple -> {
                    java.util.List<ch.hug.orthrusdast.entity.ScanJobEntity> jobs = new java.util.ArrayList<>(tuple.getT1());
                    jobs.sort((j1, j2) -> Long.compare(j2.getId(), j1.getId()));
                    java.util.List<ch.hug.orthrusdast.entity.SlaveNodeEntity> slaves = tuple.getT2();

                    java.util.Map<String, Long> activeJobsCount = new java.util.HashMap<>();
                    for (ch.hug.orthrusdast.entity.SlaveNodeEntity slave : slaves) {
                        long count = jobs.stream()
                                .filter(j -> slave.getId().equals(j.getAssignedSlaveId())
                                        && j.getStatus() == ch.hug.orthrusdast.model.JobStatus.RUNNING)
                                .count();
    ...
    public Mono<String> restartJob(@org.springframework.web.bind.annotation.PathVariable Long id) {
        return scanJobRepository.findById(id)
                .flatMap(job -> {
                    job.setStatus(ch.hug.orthrusdast.model.JobStatus.PENDING);
    ...
    public Mono<String> newScan(Model model) {
        return slaveNodeRepository.findAll().collectList().flatMap(slaves -> {
            long idleCount = slaves.stream().filter(s -> s.getStatus() == ch.hug.orthrusdast.model.NodeStatus.IDLE)
                    .count();
            long busyCount = slaves.stream().filter(s -> s.getStatus() == ch.hug.orthrusdast.model.NodeStatus.BUSY)
                    .count();
            ...
            ch.hug.orthrusdast.entity.SlaveNodeEntity activeSlave = slaves.stream()
                    .filter(s -> s.getStatus() == ch.hug.orthrusdast.model.NodeStatus.IDLE
                            || s.getStatus() == ch.hug.orthrusdast.model.NodeStatus.BUSY)
    ...
    public Mono<String> activeScans(Model model) {
        return scanJobRepository.findAllByOrderByCreatedAtDesc(org.springframework.data.domain.PageRequest.of(0, 100))
                ...
                .flatMap(history -> {
                    ...
                    for (ch.hug.orthrusdast.entity.ScanJobEntity job : history) {
                        java.util.Map<String, Object> map = new java.util.HashMap<>();
                        map.put("job", job);

                        if (job.getStatus() == ch.hug.orthrusdast.model.JobStatus.COMPLETED
                            ...
                            Mono<java.util.Map<String, Object>> m = scanResultService.findById(job.getResultId())
                                    .map(result -> {
                                        ...
                                        long critical = result.riskSummary()
                                                .getOrDefault(ch.hug.orthrusdast.model.RiskLevel.CRITICAL, 0L);
                                        long high = result.riskSummary()
                                                .getOrDefault(ch.hug.orthrusdast.model.RiskLevel.HIGH, 0L);
                                        long medium = result.riskSummary()
                                                .getOrDefault(ch.hug.orthrusdast.model.RiskLevel.MEDIUM, 0L);
                                        long low = result.riskSummary()
                                                .getOrDefault(ch.hug.orthrusdast.model.RiskLevel.LOW, 0L);
                                        String grade = "A";
                                        if (critical > 0)
                                            grade = "F";
                                        else if (high > 0)
    ...
    private int getStatusWeight(ch.hug.orthrusdast.model.JobStatus status) {
        if (status == ch.hug.orthrusdast.model.JobStatus.PENDING)
            return 1;
        if (status == ch.hug.orthrusdast.model.JobStatus.RUNNING)
            return 2;
        return 3;
    }
            ...
            return Mono.justOrEmpty(oauth2Config)
                    ...
                    .flatMap(fetchedTokens -> {
                        ...
                        try {
                            ...
                            ch.hug.orthrusdast.entity.ScanJobEntity job = new ch.hug.orthrusdast.entity.ScanJobEntity(
                                    discovererId, target, configJson, ch.hug.orthrusdast.model.JobStatus.PENDING);
                            return scanJobRepository.save(job)
