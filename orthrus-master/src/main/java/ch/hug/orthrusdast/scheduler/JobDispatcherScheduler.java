package ch.hug.orthrusdast.scheduler;

import ch.hug.orthrusdast.entity.ScanJobEntity;
import ch.hug.orthrusdast.entity.SlaveNodeEntity;
import ch.hug.orthrusdast.repository.ScanJobRepository;
import ch.hug.orthrusdast.repository.SlaveNodeRepository;
...
public class JobDispatcherScheduler {
    ...
    private final ch.hug.orthrusdast.config.OrthrusProperties orthrusProperties;
    private final ch.hug.orthrusdast.sse.JobEventPublisher jobEventPublisher;

    public JobDispatcherScheduler(ScanJobRepository scanJobRepository, SlaveNodeRepository slaveNodeRepository,
            ch.hug.orthrusdast.config.OrthrusProperties orthrusProperties, ch.hug.orthrusdast.sse.JobEventPublisher jobEventPublisher) {
        ...
        slaveNodeRepository.findAll()
                .flatMap(slave -> {
                    return webClient.get()
                            ...
                            .flatMap(isUp -> {
                                if (!isUp && slave.getStatus() != ch.hug.orthrusdast.model.NodeStatus.OFFLINE) {
                                    System.out
                                    ...
                                    return slaveNodeRepository.updateSlaveNodeStatusAndLastSeenAt(slave.getId(),
                                            ch.hug.orthrusdast.model.NodeStatus.OFFLINE.name(), slave.getLastSeenAt())
                                            .doOnSuccess(r -> System.out.println("Rows updated to OFFLINE: " + r));
                                } else if (isUp && slave.getStatus() == ch.hug.orthrusdast.model.NodeStatus.OFFLINE) {
                                    ...
                                    return slaveNodeRepository
                                            .updateSlaveNodeStatusAndLastSeenAt(slave.getId(),
                                                    ch.hug.orthrusdast.model.NodeStatus.IDLE.name(), Instant.now())
        ...
        return scanJobRepository.countByAssignedSlaveIdAndStatus(slave.getId(), JobStatus.RUNNING)
                .flatMap(runningCount -> {
                    ...
                    if (newCount >= slave.getMaxConcurrentScans()) {
                        slave.setStatus(ch.hug.orthrusdast.model.NodeStatus.BUSY);
                    } else {
                        slave.setStatus(ch.hug.orthrusdast.model.NodeStatus.IDLE);
                    }
                    return slaveNodeRepository
                ...
                .doOnSuccess(j -> {
                    jobEventPublisher.emit(j.getId(), ch.hug.orthrusdast.sse.JobEvent.running(j.getId(), j.getTarget()));
                })
                .flatMap(j -> {
